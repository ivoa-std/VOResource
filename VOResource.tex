...
is typically encapsulated in \xmlel{vr:Capa\-bility}-typed elements. This
...
elements like \vorent{publisher} or records of type
\vorent{vr:Organisation}) taken from the Research Organization
...
\item[ASCL id] An identifier for software registered in the Astrophysics
Source Code Library\footnote{\url{https://www.ascl.net/}}.  In
VOResource, these identifiers are written in URI form, as in
\verb|ascl:1101.010|.
...
\xmlel{publisher} and \xmlel{relatedResource}) make use of the
...
\begin{quotation}\noindent
% GENERATED: !vocterms voresource/date_role
\textsl{Accepted},
\textsl{Available},
\textsl{Collected},
\textsl{Copyrighted},
...
\begin{quotation}\noindent
% GENERATED: !vocterms voresource/content_level
\textsl{Amateur},
\textsl{General},
\textsl{Research}
% /GENERATED
...
\begin{quotation}\noindent
\raggedright
% GENERATED: !vocterms voresource/content_type
\textsl{Animation},
\textsl{Archive},
\textsl{Artwork},
\textsl{Background},
...
\begin{quotation}\noindent
\raggedright
% GENERATED: !vocterms voresource/relationship_type
\textsl{Cites},
\textsl{Continues},
\textsl{HasPart},
\textsl{IsContinuedBy},
...
multiple places within a \xmlel{vr:Re\-source} type or sub-type.  It may
...
\begin{admonition}{Note}
\xmlel{validationLevel} is of type \xmlel{vr:Validation}; somewhat
confusingly, VOResource also has a type \xmlel{vr:ValidationLevel}.  The
latter is extended by \xmlel{vr:Validation} with a \xmlel{validatedBy}
...
The \xmlel{validationLevel} element's attribute
\xmlel{validatedBy} takes an IVOID as a value.  This IVOID
must refer to a registered organisation or registry that assigned the
numerical value.  The \xmlel{validationlevel} element can appear
...
\begin{quotation}
\noindent A service is any VO resource that can be invoked by the user to
...
provide documentation that explains what clients should do with
\xmlel{accessURL}; this documentation should follow the
documention conventions described in sect.~\ref{sect:core}.
A general hint on what to expect when dereferencing an access URL is
also given by \xmlel{vr:AccessURL}'s \xmlel{use} attribute:

% GENERATED: !schemadoc VOResource-v1.3.xsd AccessURL
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
\item[Allowed Values]\hfil
\begin{longtermsdescription}\item[full]
                     Assume a full URL, i.e., one that can be invoked
                     directly without alteration.  This could return a
                     single, constant document, but more typically will be a
                     URL to be POSTed to.

\item[base]
                     Assume a base URL, i.e., one requiring extra material
                     to be appended before being dereferenced.

\item[dir]
                     Assume the URL points to a directory that will return a listing
                     of files.
...
covers what \xmlel{role/@altIdentifi\-er} and
...
\begin{itemize}
\item \xmlel{interface} now only allows zero or one
\xmlel{securityMethod} elements (rather than zero to unlimited in

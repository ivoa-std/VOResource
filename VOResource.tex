...
in interoperable registries and resource discovery within them.
...
The vocabulary management closely follows the one used in the managament
of the Datalink \citep{2015ivoa.spec.0617D} core vocabulary, designed by
Norman Gray.
...
\begin{description}
\item[Registry Interfaces \citep{2018ivoa.spec.0723D}] The Registry
Interfaces standard describes how registries exchange VOResource records, and it
...
\item[RegTAP \citep{2019ivoa.spec.1011D}] The Registry Relational Schema
gives a relational mapping of the models discussed here.
\item[VOSI \citep{2017ivoa.spec.0524G}] The VO support interfaces re-use
...
elements (at this point, creator and contact) is an ORCID.  In
accordance with ORCID's
instructions\footnote{\url{https://support.orcid.org/knowledgebase/articles/116780}}
(and contrary to Working Drafts of this document), ORCIDs must be given
as HTTPS URIs, for instance
...
\begin{quotation}\noindent
% GENERATED: sed -e 's/\([^;]*\);.*/\\textsl{\1},/;$s/,$//' terms/date_role.terms | sort -i
...
\begin{quotation}\noindent
% this was generated by: sed -e 's/\([^;]*\);.*/\\textsl{\1},/;$s/,$//' terms/content_level.terms
\textsl{Research},
\textsl{Amateur},
\textsl{General}
% (but we need to fix the procedure to use RDF repo, as the stuff
% is maintained there now)
\end{quotation}
...
\begin{quotation}\noindent
% this was generated by: sed -e 's/\([^;]*\);.*/\\textsl{\1},/;$s/,$//' terms/content_type.terms
\textsl{Other},
\textsl{Archive},
\textsl{Bibliography},
\textsl{Catalog},
...
\begin{quotation}\noindent
% this was generated by: sed -e 's/\([^;]*\);.*/\\textsl{\1},/;$s/,$//' terms/relationship_type.terms | sort -i
\textsl{Cites},
\textsl{Continues},
\textsl{HasPart},
\textsl{IsContinuedBy},
...
\item[Comment] 
                     ...
                     license URIs (https://spdx.org/licenses/) if at all 
                     possible.
